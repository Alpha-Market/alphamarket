...
function HostCreateGroup({
	...
	const [user] = useUser();
	const { createGroup, uploadFile } = useUserAction();
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const public_url = await uploadFile(group_photo as File, "group");
			await createGroup({
				name: group_name,
				ticker: group_ticker,
				description: group_description,
				contract_address: txRes.contractAddress!,
	...
	const isSubmitDisabled = () => {
		return !group_photo || !group_name || !group_ticker || !group_description;
	};
