...
const HostProfileScreen = () => {
    ...
    const paginate = (direction: number) => {
        if (direction === 1) {
            control.start({ x: -dragConstraintRef.current!.clientWidth, /* transition: { duration: 0 } */ });
            posRef.current = 1;
            // setI(1)
        } else {
    ...
    return (
        <div className="flex flex-col bg-black w-full h-full">
            ...
            <div className="mx-auto flex flex-col w-full max-w-[732px]">
                    ...
                    <div className="flex-1 flex flex-col gap-[14px]">
                        <div className="flex items-center gap-[6px] max-w-[110px] border border-stroke-1 rounded-[4px] p-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width={10} height={10} viewBox="0 0 10 10" fill="none">
                                <path d="M5.94784 4.23465L9.66837 -0.000488281H8.78703L5.55514 3.67607L2.97575 -0.000488281H0L3.90139 5.55965L0 10.0003H0.881334L4.29209 6.11689L7.0167 10.0003H9.99245M1.19943 0.650354H2.55341L8.78637 9.38143H7.43205" fill="white" fillOpacity="0.6" />
                            </svg>

                            <div className="w-[1px] h-[10px] bg-stroke-1" />

                            <span className="text-secondary text-xs font-medium">@Alex_Block</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1">
                            <div className="bg-card-1 py-2 px-3 rounded-[80px]">
                                <p className="text-secondary text-xs font-medium">Bitcoin</p>
                            </div>

                            <div className="bg-card-1 py-2 px-3 rounded-[80px]">
                                <p className="text-secondary text-xs font-medium">DeFi</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-3 bg-card-2 rounded-b-lg flex flex-col gap-4">
                    <p className="text-white font-medium text-xs">Spaces host, BTC, husband, and tech leader </p>

                    <div className="flex items-center gap-3">
                        <p className="text-white text-xs font-medium uppercase">
                            24.7K
                            &nbsp;
                            <span className="text-secondary text-xs font-medium uppercase">followers</span>
                        </p>
                        ...
                        <p className="text-white text-xs font-medium uppercase">
                            150
                            &nbsp;
                            <span className="text-secondary text-xs font-medium uppercase">posts</span>
                        </p>
                            ...
                            TABS.map(tab => (
                                <li
                                    ...
                                    className={cn(
                                        ...
                                        selectedTab === tab ? "text-white font-bold" : "text-secondary font-normal"
                                    )}
                                    onClick={() => setSelectedTab(tab)}
                                >
                                    {tab}
                                    {selectedTab === tab ? (
                                        <motion.div className="absolute bottom-0 inset-x-0 h-[1px] bg-white" layoutId="underline" />
                                    ) : null}
                                </li>
                            ))
                        }
                        ...
                        <motion.div
                            ...
                            onDragEnd={(e, info) => {
                                ...
                                if (swipe < -swipeConfidenceThreshold) {
                                    control.start({ x: -dragConstraintRef.current!.clientWidth });
                                    posRef.current = 1;
                                } else if (swipe > swipeConfidenceThreshold) {
                                    control.start({ x: 0 });
                                    posRef.current = 0;
                                } else {
                                    control.start({ x: -dragConstraintRef.current!.clientWidth * posRef.current });
                                }
                            }}
